...
import SubledgerRangePicker from './SubledgerRangePicker';
...
function Subsidiary({ open = false, close = () => {} }) {
    ...
    async function fetchData(){
        try {
            const response = await axios.post(`/reports/other/subsidiary`, {
                from,
                to,
                subledgers: subledgers.map(m=>m.slCode),
    ...
    async function submitClick() {
        if (accounts.length === 0 || subledgers.length === 0) {
    ...
    async function exportClick() {
        ...
        try {
            ...
            link.href = url;
            link.setAttribute('download', 'SUBSIDIARY REPORT.xlsx'); // Set filename
    ...
    return (
        <Modal show={open} closeCallback={close} title="Subsidiary Report">
            <div className="flex-1 border-t">
                <div className="p-2 flex items-end flex-wrap text-[0.9em]">
                    ...
                    <div className='mb-2'>
                        <SubledgerRangePicker selected={subledgers} setSelected={setSubledgers} />
                    </div>
