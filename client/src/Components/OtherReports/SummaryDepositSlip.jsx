...
function SummaryDepositSlip({ show=false, close=()=>{} }) {
    ...
    async function fetchData(page = 1){
        ...
        try {
            const response = await axios.get(`/reports/other/depositSlip?from=${from}&to=${to}&page=${page}`, { withCredentials: true });
    ...
    async function exportClick(){
        ...
        try {
            loading(true);
            const response = await axios.get(`/reports/other/depositSlip/export?from=${from}&to=${to}`, { withCredentials: true, responseType: 'blob' });
            loading(false);
            ...
            link.href = url;
            link.setAttribute('download', 'DEPOSIT SLIP.xlsx'); // Set filename
    ...
    return (
        <Modal show={show} closeCallback={close} title='Depost Slips'>
                            ...
                            <tbody>
                            {
                                rows.map((item, index)=>
                                    <tr key={index} className='border-b hover:bg-gray-100'>
                                        ...
                                        <td className='py-1 px-2'>{item.details}</td>
                                        <td className='py-1 px-2'>{item.date?.substr(0, 10).replaceAll("-", "/")}</td>
                                        <td className='py-1 px-2'>{item.particulars}</td>
...
export default SummaryDepositSlip;