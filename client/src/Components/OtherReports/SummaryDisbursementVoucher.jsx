...
function SummaryDisbursementVoucher({ show=false, close=()=>{} }) {
    ...
    async function fetchData(page = 1){
        ...
        try {
            const response = await axios.get(`/reports/other/disbursementVoucher?from=${from}&to=${to}&page=${page}`, { withCredentials: true });
    ...
    async function exportClick(){
        ...
        try {
            loading(true);
            const response = await axios.get(`/reports/other/disbursementVoucher/export?from=${from}&to=${to}`, { withCredentials: true, responseType: 'blob' });
            loading(false);
            ...
            link.href = url;
            link.setAttribute('download', 'DISBURSEMENT VOUCHER.xlsx'); // Set filename
    ...
    return (
        <Modal show={show} closeCallback={close} title='Disbursement Voucher'>
...
export default SummaryDisbursementVoucher;