...
function SummaryCashReceipt({ show=false, close=()=>{} }) {
    ...
    async function fetchData(page = 1){
        ...
        try {
            const response = await axios.get(`/reports/other/cashReceipt?from=${from}&to=${to}&page=${page}`, { withCredentials: true });
    ...
    async function exportClick(){
        ...
        try {
            loading(true);
            const response = await axios.get(`/reports/other/cashReceipt/export?from=${from}&to=${to}`, { withCredentials: true, responseType: 'blob' });
            loading(false);
            ...
            link.href = url;
            link.setAttribute('download', 'CASH RECEIPT.xlsx'); // Set filename
    ...
    return (
        <Modal show={show} closeCallback={close} title='Cash Receipt'>
                        ...
                        <table className='w-[100%]'>
                            <thead>
                                <tr className='border-b sticky top-0 bg-gray-100'>
                                    ...
                                    <th className='border-r p-1'>PAYOR</th>
                            ...
                            <tbody>
                            {
                                rows.map((item, index)=>
                                    ...
                                    <tr key={index} className='border-b hover:bg-gray-100'>
                                        ...
                                        <td className='py-1 px-2'>{item.payor}</td>
...
export default SummaryCashReceipt;