...
const BudgetPerAccount = () => {
  const { loading, setLoading } = useLoader();
  const [slCode, setSLCode] = useState("");
  const [name, setName] = useState("");
  const [categoryCodes, setCategoryCodes] = useState("");
  const [errorMessage, setErrorMessage] = useState("");
  const [workGroupCodes, setWorkGroupCodes] = useState([]);
  ...
  useEffect(() => {
    if (
      selectedAccount &&
      selectedAccount.code &&
      selectedAccount.code !== categoryCodes
    ) {
      setCategoryCodes(selectedAccount.code); // Set only one value
    }
  }, [selectedAccount]);
  ...
  const handleRemoveCategoryCode = () => {
    setCategoryCodes(""); // Clear the single category code
  };
  ...
  const handleFetchReport = async () => {
    ...
    try {
      ...
      const response = await BudgetTrackApi.getPerAccountReport(
        formattedStartDate,
        formattedEndDate,
        categoryCodes,
        workGroupCodes
  ...
  return (
    <>
      <div className="mx-auto p-4 overflow-auto">
        <h1 className="text-xl font-bold">Budget Per Account Report</h1>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <div className="flex items-center justify-center space-x-5">
          <div >
            <h3>Selected Category Code</h3>
            {categoryCodes ? (
              <p className="text-[0.8em]">
                {categoryCodes}{" "}
                <button
                  onClick={handleRemoveCategoryCode}
          ...
          <div className="text-[0.8em]">
            <h3>Selected WorkGroup Codes</h3>
            <ul>
              {workGroupCodes.length > 0 ? (
                workGroupCodes.map((code, index) => (
                  <li key={index}>
                    {code}{" "}
                    <button
                      onClick={() => handleRemoveWorkGroupCode(code)}
                      className="text-red-500"
                    >
                      Remove
                    </button>
              ...
              <tbody className="text-[0.7em]">
                {getCurrentPageData().map((report, index) => (
                  <tr key={index} className="bg-white">
                    ...
                    <td className="border border-gray-300 px-4 py-2">
                      {numberToCurrencyString(report.totalActual)}
                    </td>
                    <td className="border border-gray-300 px-4 py-2">
                      {numberToCurrencyString(report.totalBalance)}
                    </td>
                    <td className="border border-gray-300 px-4 py-2">
                      {numberToCurrencyString(report.CategoryPercentage)}%
                    </td>
                  </tr>
                ))}
              </tbody>
...
export default BudgetPerAccount;
