...
const JournalModalTemp = ({ isOpen, onClose, onSaveJournal, entryData, mode, canLiquidate=false }) => {
  ...
  const validateForm = () => {
    ...
    //   return false;
    // }
    const l = getRawLedgers();
    if(l.length === 0){
        showToast("Enter atleast 1 row", "warning");
        return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      let savedEntry = {}
      if (mode === "edit") {
        // replaced with temporary update
        savedEntry = await EntriesApi.updateTempEntry(entryData._id, formData);
        ...
        // replaced with temporary create
        savedEntry = await EntriesApi.createTempEntry(formData);
  ...
  async function liquidateClick(){
    ...
    try {
      ...
      if (mode === "edit") {
        ...
        console.log('AAAAAAAAAAAAAAAAAAA now move this out from temp', formData._id);
        const response = await axios.post(`/ca/CashAdvance/liquidate/${formData._id}`, {formData: formData}, { withCredentials: true });
      ...
      <div className="bg-white p-6 rounded-lg w-full m-10">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">
            {mode === "edit" ? "Edit Journal Entry (temporary)" : "Add Journal Entry (temporary)"}
          </h2>
          <button
            onClick={async () => {
            ...
            {(
              ...
              <>
                ...
                <div className="flex items-end">
                  ...
                  >
                    {mode === "edit" ? "Save Changes" : "Save Temporary Journal"}
                  </button>
                    {
                        (mode === "edit" && canLiquidate) &&
                        <button  
                            className="bg-green-500 text-white py-2 px-4 rounded-md hover:bg-green-600 ml-4"
                            type="button"
                            onClick={liquidateClick} >Liquidate</button>
                    }
                </div>
              </>
            ) : (
...
export default JournalModalTemp;
