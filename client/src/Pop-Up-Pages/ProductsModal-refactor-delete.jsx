...
const ProductsModal = ({ isOpen, onClose, mode, product, onSaveProduct }) => {
  ...
  useEffect(() => {
    if (product) {
      ...
      setProductImage(
        product.productImage ? product.productImage.split(",")[1] : ""
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Payload to update:", updatedProduct);

    try {
      let response;
      if (mode === "add") {
        response = await ProductsAndServicesApi.createProduct(updatedProduct);
        onSaveProduct({...response, account: formData.account}); // Pass the full response object instead of `newProduct`
        handleClear();
      } else if (mode === "edit") {
        response = await ProductsAndServicesApi.updateProduct(
          product._id,
          updatedProduct
        );
        onSaveProduct({...response, account: formData.account}); // Pass the updated product response
      }
  ...
  const resetFields = () => {
    ...
    setProductImage(""); // Reset the product image
    setError("");
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              onChange={handleChange}
              readOnly={!!product?.sku} // Make SKU read-only if it exists
              className={`border border-gray-300 rounded-md px-4 py-2 w-full ${
                product?.sku ? "bg-gray-200 cursor-not-allowed" : ""
              }`}
            />
            {mode === "edit" && product?.sku && (
              <p className="text-gray-500 text-sm mt-1">
                SKU exists: {product.sku} (Read-only)
              </p>
            )}
          </div>
